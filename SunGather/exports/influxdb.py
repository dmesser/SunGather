...
class export_influxdb(object):
    ...
    def configure(self, config, inverter):
        ...
        if (
            not self.influxdb_config["org"]
            or not self.influxdb_config["bucket"]
            or not (
                self.influxdb_config["token"]
                or (
                    self.influxdb_config["username"]
                    and self.influxdb_config["password"]
                )
            )
        ):
        ...
        for measurement in config.get("measurements"):
            if not inverter.validateRegister(measurement["register"]):
                logging.error(
                    f"InfluxDB: Configured to use {measurement['register']} but not configured to scrape this register"
                )
                continue
    ...
    def publish(self, inverter):
        ...
        for measurement in self.influxdb_measurements:
            ...
            if (
                not inverter.validateLatestScrape(register)
                or not inverter.validateRegister(register)
                or inverter.getRegisterValue(register) is None
            ):
                logging.info(
                    f"InfluxDB: Skipping data point, {register} missing from last scrape"
                )
                continue

            value = (
                inverter.getRegisterValue(register)
                if type(inverter.getRegisterValue(register)) is str
                else float(inverter.getRegisterValue(register))
            )
            sequence.append(
                influxdb_client.Point(measurement["point"])
                .tag("inverter", inverter.getInverterModel(True))
                .tag("serial", inverter.getSerialNumber())
                .field(register, value)
            )

        try:
            self.write_api.write(
                self.influxdb_config["bucket"], self.client.org, sequence
            )
        except Exception as err:
